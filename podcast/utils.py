...
class Parser:
    ...
    def get_podcast_data(self):
        ...
        try:
            t = podcast.url
            t = podcast.lastBuildDate
            t = podcast.pubDate
        except:
            close_pattern = f"(?:<itunes:image+)(.*?)(?:/>)"
            image_set = re.findall(close_pattern, str(rss_excluded_episodes))
            key_value_pattern = '([^ ]*?)=\"(.*?)\"'
            image_value = re.findall(key_value_pattern, str(image_set))
            podcast.url = image_value[0][1]
            podcast.lastBuildDate = "Tue, 05 Sep 2023 08:00:00 -0000"
